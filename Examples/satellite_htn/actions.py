...
def calibrate(s, sat, i, d):
    ...
    Returns:
        The updated state.
    """
    if (s.on_board[i] == sat) and (d in s.calibration_target[i]) and (s.pointing[s] == d) and (s.power_on[i] == True):
        s.calibrated[i] = True
        return s
...
def take_image(s, sat, d, i, m):
    ...
    """
    if (s.calibrated[i] == True) and (s.on_board[i] == sat) and (m in s.supports[i]) and (s.power_on[i] == True) and (s.pointing[s] == d) and (s.data_capacity[s] >= s.data[(d, m)]):
        s.data_capacity[s] -= s.data[(d, m)]
